...
```
함수 선언문은 런타임 이전에 자바스크립트 엔진에 의해 함수 이름과 동일한 이름의 식별자가 암묵적으로 생성되어서 (=함수 객체가 먼저 생성된다) 런타임에는 이미 할당까지 완료된 상태이므로 코드의 맨 선두에 실행된 것처럼 동작한다. 이를 함수 호이스팅이라고 한다.

반면 함수 표현식은 변수에 할당된 값이 함수 리터럴인 문이다. 변수 선언은 런타임 이전에 undefined로 초기화되지만 변수 할당문의 평가는 런타임에 실행되므로 함수 표현식의 함수 리터럴도 런타임에 실행된다. 
따라서 여기엔 변수 호이스팅이 발생된다.
